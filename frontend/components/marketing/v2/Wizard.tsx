  ...
  return (
    <div className="wizard-demo">
      <div>
        ...
        <h3 style={{ fontSize: 22, marginBottom: 8 }}>Three steps. Ten seconds.</h3>
        <p style={{ color: "var(--ink-2)", fontSize: 14, marginBottom: 32, lineHeight: 1.5 }}>
          Tell us the shape of your room, the feeling you&apos;re after, and what you&apos;d like more of. We handle the rest.
        </p>
        <div className="wizard-stepper">
          <button className={`wizard-step ${step === 1 ? "active" : step > 1 ? "done" : ""}`} onClick={() => setStep(1)}>
            ...
            <span className="wizard-step-value">{dims.w}×{dims.l}m</span>
          </button>
          ...
          <button className={`wizard-step ${step === 3 ? "active" : ""}`} onClick={() => setStep(3)}>
      ...
      <div className="wizard-panel">
        {step === 1 && <Step1Dims dims={dims} setDims={setDims} />}
        {step === 2 && <Step2Style style={style} setStyle={setStyle} />}
        {step === 3 && <Step3Prefs prefs={prefs} togglePref={togglePref} />}
...
function Step1Dims({ dims, setDims }: { dims: Dims; setDims: (fn: (d: Dims) => Dims) => void }) {
  return (
    <>
      <div className="wizard-panel-eyebrow">Step 1 of 3</div>
      <h3>How big is the room?</h3>
      <p className="wizard-panel-sub">Approximate is fine. We&apos;ll round to sensible bounds.</p>
      <div className="dim-grid">
        <DimInput label="Width" value={dims.w} unit="m" min={3} max={10} step={0.1}
          onChange={(v) => setDims((d) => ({ ...d, w: v }))} />
        <DimInput label="Length" value={dims.l} unit="m" min={3} max={10} step={0.1}
          onChange={(v) => setDims((d) => ({ ...d, l: v }))} />
        <DimInput label="Height" value={dims.h} unit="m" min={2.4} max={4} step={0.1}
...
function Step2Style({ style, setStyle }: { style: string; setStyle: (v: string) => void }) {
  return (
    <>
      <div className="wizard-panel-eyebrow">Step 2 of 3</div>
      <h3>Pick a style</h3>
...
function Step3Prefs({ prefs, togglePref }: { prefs: string[]; togglePref: (id: string) => void }) {
  return (
    <>
      <div className="wizard-panel-eyebrow">Step 3 of 3</div>
